...
enum DndClass:
  ...
  def action: CombatAction =
    this match
      ...
      case DndClass.WIZARD(_) => CombatAction(1, D6)

  def bonusAction: Option[CombatAction] =
    this match
      case DndClass.PALADIN(lvl) => if lvl > 3 then Some(CombatAction(1, D6)) else None
      case DndClass.ROGUE(lvl) => if lvl > 2 then Some(CombatAction(2, D6)) else None
      case DndClass.WIZARD(lvl) => if lvl > 1 then Some(CombatAction(1, D6)) else None
